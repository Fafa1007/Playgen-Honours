...
with torch.no_grad():
    # Start from first ground-truth frame
    img, _ = dataset[0]
    ...
    for idx, action in enumerate(dataset.actions):
        ...
        # Decide whether to use prediction or ground truth
        if (idx + 1) % 8 == 0:  
            # Every 5th step → reset to ground truth
        ...
        # Instead of using prediction → feed next ground-truth frame
        # i.e., skips and z_t come from the *real* frame[idx+1]
